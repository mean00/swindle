...
OPT="--target=armv7m-none-eabi -mcpu=cortex-m3"
...
SYS=${TOOLCHAIN}/../arm/llvm_21/lib/clang-runtimes/arm-none-eabi/armv7m_soft_nofp/include
build_stub() {
  rm -f $1.o
  set -x
  ...
  $OBJDUMP -S $1.o >$1.asm
  xxd -i $1.bin >$1.tmp
  cat $1.tmp | sed 's/unsigned char/const unsigned char/g' | sed 's/^.*_len =/#define GD32_CRC32_BIN_LEN/g' >$1.stub
}

build_stub gd32_crc32
