//! Auto-generated FFI bindings to the C BMP (Black Magic Probe) command layer.
//!
//! Generated by `rust-bindgen 0.72.1` from the C headers. Provides `extern "C"`
//! function declarations for:
//!
//! - Target memory read/write (`bmp_mem_read_c`, `bmp_mem_write_c`)
//! - Register access (`bmp_read_register_c`, `bmp_write_reg_c`, etc.)
//! - Flash operations (`bmp_flash_erase_c`, `bmp_flash_write_c`, `bmp_flash_complete_c`)
//! - SWD/RISC-V debug protocol (`bmp_rpc_swd_*_c`, `bmp_rv_*_c`)
//! - ADIv5 AP/DP access (`bmp_adiv5_*_c`)
//! - Breakpoint/watchpoint management (`bmp_add_breakpoint_c`, `bmp_remove_breakpoint_c`)
//! - Target control (`bmp_target_halt_c`, `bmp_target_halt_resume_c`, `bmp_poll_target_c`)
//! - Platform I/O (`platform_buffer_read`, `platform_buffer_write`, `platform_write_flush`)
//! - RTT info (`bmp_rtt_get_info_c`, `bmp_rtt_set_info_c`)
//! - Exception handling (`bmp_try_c`, `bmp_catch_c`, `bmp_raise_exception_c`)
//!
//! # Safety
//!
//! All functions are `unsafe` — the caller must ensure pointers are valid,
//! buffers are properly sized, and the target is in the expected state.
...
pub const BMP_ARCH_NONE: u32 = 0;
pub const BMP_ARCH_ARM: u32 = 1;
pub const BMP_ARCH_RISCV: u32 = 2;
